...
bool I2C::perform_I2C_job(uint8_t type, uint8_t addr, uint8_t reg, uint8_t *buf, uint8_t len, volatile uint8_t *status, void (*CB)())
{
    ...
    else if(type & POLLING)
    {
        ...
        if(reg_ != 0xFF)
        {
            ...
            while ((I2Cx_->SR1 & I2C_SR1_SB) != I2C_SR1_SB && --timeout > 0);
            if (timeout == 0)
                return hardware_failure();
            ...
            uint64_t t1 = micros();
            while (!(I2Cx_->SR1 & I2C_SR1_ADDR) && --timeout > 0);
            uint64_t t2 = micros();
            if (timeout == 0)
                // This means that there is no device on this address
                return false;
        ...
        else
        {
            ...
            I2Cx_->CR1 |= I2C_CR1_START;
            /* Wait until SB flag is set: EV5 */
            while (!(I2Cx_->SR1 & I2C_SR1_SB) && --timeout > 0);
            if (timeout == 0)
                return hardware_failure();
        }
        ...
        if(type & WRITE)
        {
            ...
            do
            {
                /* Write the data in DR register (EV8_1) */
        ...
        else // type & READ
        {
            ...
            {
                timeout = I2C_DEFAULT_TIMEOUT;
                /* Send START condition (may be a repeated start condition if subaddress was sent) */
                I2Cx_->CR1 |= I2C_CR1_START;
                /* Wait until SB flag is set: EV5  */
                while (!(I2Cx_->SR1 & I2C_SR1_SB) && --timeout > 0);
                if (timeout == 0)
            ...
            else if (len_ == 2)
            {
                ...
                /* Wait until ADDR is set: EV6 */
                timeout = I2C_DEFAULT_TIMEOUT;
                while ((I2Cx_->SR1 & I2C_SR1_ADDR) != I2C_SR1_ADDR && --timeout > 0);
                if (timeout == 0)
                    return hardware_failure();
                ...
                /*Re-enable IRQs */
                __enable_irq();
                /* Wait until byte transfer is finished (BTF is set) */
                timeout = I2C_DEFAULT_TIMEOUT;
                while (!(I2Cx_->SR1 & I2C_SR1_BTF))
                {
                    if(timeout-- == 0)
...

            {
                ...
                while (len_)
                {
                    ...
                    if (len_ != 3)
                    {
                        // acknowledge all of these bytes
                        I2C_AcknowledgeConfig(I2Cx_, ENABLE);
                    ...
                    if (len_ == 3)
                    {
                        ...
                        while ((I2Cx_->SR1 & I2C_SR1_RXNE) != I2C_SR1_RXNE);
                        /* Read DataN */
                        data_buffer_[index_] = I2Cx_->DR;
                ...
                /* Make sure that the STOP bit is cleared by Hardware before CR1 write access */
                while ((I2Cx_->CR1 & I2C_CR1_STOP) == I2C_CR1_STOP);
                /* Enable Acknowledgement for the next bytes */
                I2C_AcknowledgeConfig(I2Cx_, DISABLE);
            }
